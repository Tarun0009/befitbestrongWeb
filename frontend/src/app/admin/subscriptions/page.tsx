...
export default function AdminSubscriptionsPage() {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    const allowedFrequencies = parseFrequencies();
    if (!allowedFrequencies.length) { setError("Enter at least one frequency between 7 and 365 days."); return; }
    try {
      if (editing) await updatePlan({ id: editing.id, body: { name: form.name.trim(), discountPercent: Number(form.discountPercent), allowedFrequencies, active: form.active } }).unwrap();
      else await createPlan({ name: form.name.trim(), variantId: form.variantId, discountPercent: Number(form.discountPercent), allowedFrequencies, active: form.active }).unwrap();
  ...
  return <div className="space-y-8">
    <section className="rounded-xl border border-border p-5 sm:p-6">
      ...
      <form onSubmit={handleSubmit} className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="sm:col-span-2 lg:col-span-3"><button type="submit" disabled={creating || updating} className="rounded-lg bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground disabled:opacity-50">{creating || updating ? "Saving…" : editing ? "Update plan" : "Create plan"}</button></div>
      </form>
    </section>

    <section><div className="flex flex-wrap items-end justify-between gap-4"><div><h2 className="text-xl font-semibold">Subscription operations</h2><p className="mt-1 text-sm text-muted-foreground">The hourly worker creates reminder records without charging or reserving stock.</p></div><button type="button" onClick={handleProcess} disabled={processing} className="rounded-lg border border-border px-4 py-2 text-sm font-semibold disabled:opacity-50">{processing ? "Scanning…" : "Run due scan"}</button></div>
    ...
    <section><div className="flex items-end justify-between"><div><h2 className="text-xl font-semibold">Plans</h2><p className="mt-1 text-sm text-muted-foreground">Savings are snapshotted when a customer enrolls.</p></div><span className="text-sm text-muted-foreground">{data?.plans.length ?? 0} total</span></div>
...
const inputClass="mt-1.5 h-11 w-full rounded-lg border border-border bg-background px-3 text-sm outline-none focus:ring-2 focus:ring-primary/30 disabled:opacity-60";